...
var deleteFromMenuHelper = function (target) {
  if (target.data('new') == 1 || target.data('new') == 0) {
...
var addToMenu = function () {
  //Lâm thêm vào để fixed lỗi
  var maxId = 0;
  $('.dd-item').each(function(index, el) {
    var dataId = $(this).attr('data-id').split('-')[1];
    if(dataId > maxId)
      maxId = dataId;
  });
  newIdCount = parseInt(maxId) + 1;
  //Kết thúc Lâm thêm vào
  ...
  nestableList.append(
    ...
    'data-slug="' + newSlug + '" ' +
    'data-new="1" ' +
    'data-deleted="0" ' + 'data-children="null">' +
