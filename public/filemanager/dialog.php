				...
				if(in_array($file_array['extension'], $ext_img)){
					...
					if($ftp){
						...
						*/
						//Lam edited
						if(!file_exists($src_thumb) ){
							if(!create_img($file_path, $creation_thumb_path, $config['width_thumbnail'], $config['height_thumbnail'],'auto',$config)){
								$src_thumb=$mini_src="";
							}
						}
						//check if is smaller than thumb
						list($img_width, $img_height, $img_type, $attr)=@getimagesize($file_path);
						if($img_width<$config['width_thumbnail'] && $img_height<$config['height_thumbnail']){
							$src_thumb=$file_path;
							$show_original=true;
						}

						if($img_width<$config['width_thumbnail']/2 && $img_height<$config['height_thumbnail']/2){
							$mini_src=$current_path.$rfm_subfolder.$subdir.$file;
							$show_original_mini=true;
						}
					}
