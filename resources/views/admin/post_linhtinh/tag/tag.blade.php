...
@section('title', 'Quản lý Thẻ cho bài viết')
@section('PageContent')
	<h1 class="ui header centered page-title">Quản Lý Thẻ Của Bài Viết</h1>
  @if (session('status'))
  ...
  <table id="data-table" class="cell-border hover order-column" style="width:100%">
      ...
      <tbody>
        ...
        <tr>
          <td>{{$tag->id}}</td>
          <td class="title-column"><a href="{{url('admin-dashboard/posttags/' . $tag->id . '/edit')}}" title="sửa {{$tag->tag_name}}">{{$tag->tag_name}}</a></td>
          <td>{{$tag->tag_slug}}</td>
          <td>{{$tag->tag_seo_title}}</td>
          <td>{{$tag->tag_seo_description}}</td>
          <td>
            <a href="#" class="mini ui icon green button" title="Xem" target="_blank"><i class="eye icon"></i></a>
            <a href="{{url('admin-dashboard/posttags/' . $tag->id . '/edit')}}" class="mini ui icon blue button" title="Sửa"><i class="edit icon"></i></a>
...
@endsection
@section('JsLibraries')
  <script src="{{url('js/admin/post/tag/tag.js')}}"></script>
@endsection