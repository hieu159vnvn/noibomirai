...
@section('PageContent')
	...
	<div class="ui two column centered grid wrap-content-header">
		<div id="content-header" class="thirteen wide column">
			...
			<h1 class="ui header centered page-title">Sửa Chuyên Mục: {{$category->category_name}}</h1>
		</div>
	</div>
		...
		<div class="thirteen wide column">
			...
			<form class="ui form" action="" method="post">
				...
				<div class="field">
					...
					<input type="text" id="category-name" name="categoryName" placeholder="Tên chuyên mục" value="{{ $category->category_name }}" required>
				</div>
				<div class="field">
					<label>Slug (<span id="generate-slug">click để tự động tạo slug theo tên chuyên mục</span>)</label>
					<input type="text" id="category-slug" name="categorySlug" placeholder="slug viết không dấu và các từ cách nhau bằng dấu - , ví dụ: chuyen-muc-1,...." value="{{ $category->category_slug }}" required>
				</div>
				...
				<div class="field">
					...
					<input type="text" class="seo-title" name="seoTitle" value="{{ $category->category_seo_title }}" placeholder="SEO title">
				</div>
				<div class="field">
					<label>SEO Description  (Tối đa nên ít hơn 210 ký tự)</label>
					<p class="seo-description-messager"></p>
					<textarea rows="4" class="seo-description" name="seoDescription" placeholder="SEO description">{{ $category->category_seo_description }}</textarea>
				</div>
				<a href="{{url('admin-dashboard/postcategories')}}" class="ui labeled icon button btn-align-left"><i class="arrow left icon"></i>Danh Sách Chuyên Mục</a>
