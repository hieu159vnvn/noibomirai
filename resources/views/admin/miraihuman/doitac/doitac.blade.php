  ...
  <div class="sub header">
	...
	<div class="ui secondary menu">
      ...
      <div class="right menu">
        <div class="item">
          <a href="{{url('doitac-miraihuman/add')}}" class="ui labeled icon green button"><i class="plus circle icon"></i>Thêm</a>
        </div>
      </div>
      {{-- @endpermission --}}
  </div> 
          ...
          <tbody>
            ...
            @foreach($doitac as $key => $dt)
              <tr>
                <td>{{$key+1}}</td>
                <td>
                  @if ($dt->image == '0' || $dt->image == null)
                    <img src="uploads/no-image.jpg" alt="logo" style="max-height: 50px;">
                  @else
                    <img src="{{$dt->image}}" alt="logo" style="max-height: 50px;">
                  @endif 
                </td>
                <td>{{$dt->ten}}</td>
                <td>{{$dt->tenjp}}</td>
                <td><div class="ui form wrapstatus"  >
                    <div class="inline field status" >
                      <div class="ui toggle checkbox cont{{$dt->id}}" >
                        <input name="stt" type="checkbox" class="cont" idstatus="{{$dt->id}}" rootstatus="{{$dt->stt}}" @if ($dt->stt == 1) checked="" @endif>
                ...
                {{-- @permission('logo-edit')--}}
                  <a href="{{url('doitac-miraihuman/' . $dt->id . '/edit')}}" class="mini ui icon blue button" title="Sửa"><i class="edit icon"></i></a>
                {{-- @endpermission
...
<script>
  ...
  <script> status_delete('doitac-miraihuman'); </script>
