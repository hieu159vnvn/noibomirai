...
@section('title', 'Sửa hình ảnh trang {{$item->page}}')
@section('PageContent')
	<div class="ui two column centered grid wrap-content-header">
		<h1 class="ui header centered page-title">Sửa hình ảnh trang {{$item->page}}</h1>
	</div>	
		...
		<div class="fifteen wide column">
			...
			<div class="ui secondary menu">
				...
				<div class="right menu">
					<div class="item">
					<a href="{{url('item/add')}}" class="ui labeled icon button"><i class="plus circle icon"></i>Thêm mới</a>
					</div>
				</div>
				{{-- @endpermission --}}
			</div> 
			...
			<form class="ui form" action="" method="post" name="form_1">
				...
				<div class="field thongtin">			
					<div class="field">
						...
						<br>
						@if ($item->image)
							<img src="{{url($item->image)}}" alt="" id="prev_img" class="medium  ui image bordered img-thumbnail">
						@else
							<img src="{{url('uploads/no-image.jpg')}}" alt="" id="prev_img" class="medium ui image bordered img-thumbnail ">
						@endif						
							<input name="image" type="hidden" value="{{$item->image}}" id="none_img" class="form-control">
					</div>
				</div>		
				<div class="ui ">
					<div class="eight wide column">
						<a href="{{url('bannerpage')}}" class="ui labeled icon button btn-align-left"><i class="arrow left icon"></i>Danh Sách</a>
