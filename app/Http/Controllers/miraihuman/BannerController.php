...
{        
    ...
    public function postAddBanner(Request $request){
        ...
        if($this->saveBanner($request,$banner,"add")){
            ...
            return redirect('banner/'.$lastid->id.'/edit')->with('status', 'Đã thêm thành công!');
        }
        return redirect('banner')->with('status','Đã thêm không thành công!');
    ...
    public function postEditBanner(Request $request,$id){
        ...
        if($this->saveBanner($request,$banner,"edit")){
            return redirect('banner/'.$banner->id.'/edit')->with('status','Đã sửa thành công!');
        }
        return redirect('banner/'.$banner->id.'/edit')->with('status','Đã sửa không thành công!');
    }
