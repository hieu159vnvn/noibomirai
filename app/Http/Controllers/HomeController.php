...
if (Auth::check()) {   
    ...
    if (Session::has('key')){
        session()->put('key', 'value 1' );
        session()->save();
    }else{
        session()->put('key', 'value 0' );
        session()->save();
        $q = DB::table('historys')->insert(['id_user' => $name ,'tm' => $now, 'ip' => $ip,'create_at' => date('Y/m/d')]);
    }   
} 
        ...
        $doitac = DoiTac::all();
        $history = History::take(50)->orderByRaw('tm DESC')->get();
        $donhang = DonHang::all();
